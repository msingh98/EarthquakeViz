...
void App::onCursorMove(const VRCursorEvent &event) {
    ...
    if (mouseDown){
        ...
        // Former *TODO*: Update the "rotation" matrix based on how the user has dragged the mouse
        // Note: the mouse movement since the last frame is stored in dxy.
        vec3 movement(-1*dxy.y, -1*dxy.x,0);
        if(length(movement) != 0.0){
            rotation = toMat4(angleAxis(-1*length(movement)/350, normalize(movement)))* rotation;
        }
...
void App::onRenderGraphicsScene(const VRGraphicsState &renderState) {
    ...
    for (int x=start; x<end; x++) {
        ...
        // Former *TODO*: Draw earthquake e
        vec3 location = earth->getPosition(e.getLatitude(), e.getLongitude());
        //cout<< glm::to_string(location)<<endl;
        double life = clamp((currentTime - e.getDate().asSeconds())/PLAYBACK_WINDOW, 0.0, 1.0);
        double m = e.getMagnitude() - 5;
        Sphere sphere(location, (1-life)*(pow(m,1.5))/150, vec4(m/2, m/2-1, m/2-2, 1.0));
